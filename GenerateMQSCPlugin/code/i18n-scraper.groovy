...
def readProps = { propXml ->
    def prop = propXml."property-ui"
    def propLabel = prop.@label.text()?.trim()
    def propDescAttr = prop.@description.text()?.trim()
    def propDesc = prop.description.text()?.trim()
    propXml."value".each { valueXml ->
        def propValue = valueXml.@label.text()
        properties.add(propValue)
    }
    properties.add(propLabel)
    properties.add(propDescAttr)
...
def scrape = { pluginXmlFile ->
    ...
    slurper."property-group".each { propGroup ->
        ...
        def propGroupType = propGroup.@type.text()
        properties.add(propGroupName)
        properties.add(propGroupType)
